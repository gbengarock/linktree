        ...
        <form method='POST' action='https://getform.io/f/f3fc1e76-d936-409a-be62-21ed74ddfc74' className='flex flex-col max-w-[600px] w-full my-4'>
            <div className='sm:flex my-2'>
                <div className='flex flex-col sm:mr-10'>
                    ...
                    <input name='first_name' id='first_name' type='text' placeholder='Enter your first name' className='py-2 px-3 rounded-lg items-center border focus:outline-[#B4CAFF] border-gray-300 shadow-sm gap-2 sm:w-[340px] sm:h-[44px] text-sm' />
                    {/* <p className='text-[red] text-xs my-1'>{formErrors.first_name}</p> */}
                </div>
                <div className='flex flex-col'>
                    <label className='text-xs text-gray-700 sm:text-sm font-medium my-2'>Last name</label>
                    <input name='last_name' id='last_name' type='text' placeholder='Enter your last name' className='py-2 px-3 rounded-lg items-center border focus:outline-[#B4CAFF] border-gray-300 shadow-sm gap-2 sm:w-[340px] sm:h-[44px] text-sm' />
                    {/* <p className='text-[red] text-xs my-1'>{formErrors.last_name}</p> */}
                </div>
            </div>
            <div className='flex flex-col my-2'>
                <label className='text-xs text-gray-700 sm:text-base font-medium my-2'>Email</label>
                <input name='email' id='email' type='email' placeholder='yourname@email.com' className='p-2 border focus:outline-[#B4CAFF] rounded-lg border-gray-300 shadow-sm gap-2 py-2 px-3 items-center text-sm sm:w-[720px] sm:h-[44px]' />
                {/* <p className='text-[red] text-xs my-1'>{formErrors.email}</p> */}
            </div>
            <div className='flex flex-col my-2'>
                <label className='text-xs text-gray-700 sm:text-base font-medium my-2'>Message</label>
                <textarea name='message' id='message'className='border focus:outline-[#B4CAFF] rounded-lg border-gray-300 shadow-sm gap-2 py-2 px-3 items-center text-sm sm:w-[720px]' rows='6' placeholder="Send me a message and I'll reply you as soon as possible..." />
                {/* <p className='text-[red] text-xs my-1'>{formErrors.message}</p> */}
            </div>
